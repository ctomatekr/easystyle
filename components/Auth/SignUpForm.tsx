...
const SignUpForm: React.FC<SignUpFormProps> = ({ onSuccess, onSwitchToLogin, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors(err || { general: ['회원가입에 실패했습니다. 다시 시도해주세요.'] });
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              required
              className={`w-full px-4 py-3 bg-slate-700 border ${getFieldError('password') ? 'border-red-500' : 'border-slate-600'} rounded-lg text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-amber-400`}
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              required
              className={`w-full px-4 py-3 bg-slate-700 border ${getFieldError('password_confirm') ? 'border-red-500' : 'border-slate-600'} rounded-lg text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-amber-400`}
              placeholder="비밀번호 확인"
            />
